...
class _IdentificationScreenState extends State<IdentificationScreen> {
  ...
  void initState() {
    ...
    _classifierService = ClassifierService();  // Initialiser le service de classification
    _classifierService.loadModel();  // Charger le modèle de classification
  }

  // Initialiser la caméra
  void _initializeCamera() async {
    final cameras = await availableCameras();  // Récupérer les caméras disponibles
      ...
      body: _isCameraInitialized
          ? Stack(
              children: [
                ...
                if (_isPhotoTaken && _imageFile != null)
                  Positioned.fill(
                    child: Image.file(
                      ...
                      fit: BoxFit.cover, // S'assurer que l'image occupe tout l'espace
                    ),
                  )
                else
                  Positioned.fill(
                ...
                if (!_isPhotoTaken)
                  Positioned(
                    bottom: 90,  // Positionné juste au-dessus de la barre de navigation
                    left: MediaQuery.of(context).size.width * 0.5 - 35,  // Centrage horizontal
                    child: FloatingActionButton(
                      onPressed: _takePicture,  // Prendre une photo
                      child: Icon(Icons.camera_alt, size: 30),  // Icône de capture
                      backgroundColor: Colors.white,
                      foregroundColor: Colors.black, // Meilleur contraste sur un fond blanc
                      elevation: 10,  // Ajoute une ombre pour un effet plus visible
                    ),
                  ),
              ],
            )
